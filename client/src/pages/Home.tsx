...
function ConsoleView({ onAction, serverName }: { onAction: (action: string) => void; serverName: string }) {
  ...
  const commandMutation = trpc.node.command.useMutation({ onSuccess: result => onAction(result.output ? "Command completed" : "Command sent"), onError: error => onAction(error.message) });
  const liveLogs = logsQuery.data?.logs?.split("\n").filter(Boolean) ?? [];
  const sendCommand = (input: HTMLInputElement) => { const command = input.value.trim(); if (!command) return; commandMutation.mutate({ name: serverName, command }); input.value = ""; };
  return <div className="console-layout">
    ...
    <div className="console-window"><div className="console-window-bar"><div className="window-dots"><i /><i /><i /></div><span>{serverName} / console</span><button onClick={() => { navigator.clipboard?.writeText(logsQuery.data?.logs ?? ""); onAction("Console output copied"); }}><Copy size={13} />Copy output</button></div><div className="console-output">{logsQuery.isLoading ? <div className="log-line"><span className="log-message">Loading live Docker output…</span></div> : logsQuery.error ? <div className="log-line log-line-error"><span className="log-message">Deployment error: {logsQuery.error.message}</span></div> : liveLogs.length ? liveLogs.map((line, i) => <div className="log-line" key={`${line}-${i}`}><span className="log-message">{line}</span></div>) : <div className="log-line"><span className="log-message">No container output yet. Set one Startup command for this server, then start it.</span></div>}<div className="log-line log-cursor"><span className="log-message">_</span></div></div><div className="console-command"><span className="prompt">$</span><input placeholder="Send a command to the server..." onKeyDown={(event) => { if (event.key === "Enter") sendCommand(event.currentTarget); }} /><button onClick={(event) => sendCommand(event.currentTarget.previousElementSibling as HTMLInputElement)} disabled={commandMutation.isPending}><ArrowUpToLine size={15} /></button></div></div>
...
function FilesView({ onAction, serverName }: { onAction: (action: string) => void; serverName: string }) {
  ...
  const filesQuery = trpc.files.list.useQuery({ serverName }, { refetchInterval: 4000 });
  const uploadMutation = trpc.files.upload.useMutation({ onSuccess: result => { void utils.files.list.invalidate({ serverName }); onAction(`Uploaded ${result.path} without renaming it`); }, onError: e => onAction(`Upload error: ${e.message}`) });
  const extractMutation = trpc.files.extract.useMutation({ onSuccess: () => { void utils.files.list.invalidate({ serverName }); onAction("Archive extracted"); }, onError: e => onAction(`Extract error: ${e.message}`) });
  const actionMutation = trpc.files.action.useMutation({ onSuccess: () => { void utils.files.list.invalidate({ serverName }); onAction("File operation completed"); }, onError: e => onAction(`File operation error: ${e.message}`) });
  ...
  const uploadFile = (file: File) => { if (file.size > 25 * 1024 * 1024) return onAction("Files must be 25 MB or smaller"); const reader = new FileReader(); reader.onload = () => { if (typeof reader.result === "string") uploadMutation.mutate({ serverName, fileName: file.name, relativePath: file.name, mimeType: file.type || "application/octet-stream", size: file.size, dataUrl: reader.result }); }; reader.readAsDataURL(file); };
  const runBulk = async (operation: "move" | "copy" | "delete") => {
    if (!selected.length) return onAction("Select at least one file first");
    if (operation === "delete" && !window.confirm(`Delete ${selected.length} selected file${selected.length === 1 ? "" : "s"} permanently?`)) return;
    const destination = operation === "delete" ? "" : window.prompt(operation === "move" ? "Move selected files into folder" : "Copy selected files into folder", "");
    if (operation !== "delete" && destination === null) return;
    try { await Promise.all(selected.map(source => actionMutation.mutateAsync({ serverName, action: operation, source, destination: destination || undefined }))); setSelected([]); onAction(`${operation[0].toUpperCase()}${operation.slice(1)} completed for ${selected.length} file${selected.length === 1 ? "" : "s"}`); } catch (error) { onAction(`Bulk operation error: ${error instanceof Error ? error.message : "request failed"}`); }
  };
  const renameOne = () => { if (selected.length !== 1) return onAction("Select exactly one file to rename"); const destination = window.prompt("New file name or path", selected[0]); if (!destination) return; actionMutation.mutate({ serverName, action: "rename", source: selected[0], destination }); setSelected([]); };
  const extractSelected = async () => { const archives = selected.filter(file => file.toLowerCase().endsWith(".zip")); if (!archives.length) return onAction("Select one or more ZIP files to extract"); try { await Promise.all(archives.map(file => extractMutation.mutateAsync({ serverName, fileName: file }))); setSelected([]); onAction(`Extracted ${archives.length} ZIP archive${archives.length === 1 ? "" : "s"}`); } catch (error) { onAction(`Extract error: ${error instanceof Error ? error.message : "request failed"}`); } };
  return <div className="subview"><div className="subview-toolbar"><div className="pathbar"><Folder size={15} /><span>/</span><strong>home/container</strong></div><div className="toolbar-actions"><input ref={fileInput} type="file" multiple hidden onChange={e => { for (const file of Array.from(e.target.files ?? [])) uploadFile(file); e.currentTarget.value=""; }} /><button className="outline-button" onClick={() => fileInput.current?.click()} disabled={uploadMutation.isPending}><Upload size={14} />{uploadMutation.isPending ? "Uploading…" : "Upload files"}</button><button className="outline-button" onClick={() => { setSelected(allSelected ? [] : files.map(file => file.path)); }}><CheckSquare size={14} />{allSelected ? "Clear selection" : "Select all"}</button></div></div><div className="file-tools"><div className="file-search"><Search size={14} /><input placeholder="Filter files…" /></div><span>{filesQuery.isLoading ? "Loading container files…" : `${files.length} files · ${selected.length} selected`}</span></div><div className="file-bulk-toolbar"><strong>{selected.length ? `${selected.length} selected` : "Select files to manage them"}</strong><button className="outline-button" onClick={renameOne}><FileCode2 size={14} />Rename</button><button className="outline-button" onClick={() => runBulk("move")}><ArrowDownToLine size={14} />Move</button><button className="outline-button" onClick={() => runBulk("copy")}><Copy size={14} />Copy</button><button className="outline-button" onClick={extractSelected}><FileArchive size={14} />Unzip</button><button className="outline-button danger-action" onClick={() => runBulk("delete")}><Trash2 size={14} />Delete</button></div>{filesQuery.isLoading ? <div className="storage-loading"><RefreshCw size={17} className="spin-icon" />Loading files…</div> : files.length === 0 ? <div className="empty-state storage-empty"><div><Folder size={21} /></div><strong>Container is empty</strong><p>Upload a ZIP or project file, then extract it and choose one explicit startup command.</p><button onClick={() => fileInput.current?.click()}>Upload project <Upload size={14} /></button></div> : <div className="data-table file-table"><div className="data-row data-head"><span>Select</span><span>Name</span><span>Size</span><span>Modified</span><span>Actions</span></div>{files.map(file => <div className={`data-row ${selected.includes(file.path) ? "data-row-selected" : ""}`} key={file.path}><span><button className="select-file-button" aria-label={`Select ${file.path}`} onClick={() => toggle(file.path)}>{selected.includes(file.path) ? <CheckSquare size={16} /> : <Square size={16} />}</button></span><span className="file-name"><span className="file-icon file-icon-code"><File size={16} /></span><strong>{file.path}</strong></span><span className="muted-text">{formatBytes(file.size)}</span><span className="muted-text">{new Date(file.modifiedAt).toLocaleString()}</span><span className="file-row-actions">{file.path.toLowerCase().endsWith(".zip") && <button className="row-more visible-file-action" title="Extract ZIP" onClick={() => extractMutation.mutate({ serverName, fileName: file.path })}><FileArchive size={15} />Unzip</button>}<button className="row-more visible-file-action" title="Rename" onClick={() => { setSelected([file.path]); renameOne(); }}>Rename</button><button className="row-more visible-file-action" title="Move" onClick={() => { setSelected([file.path]); runBulk("move"); }}>Move</button><button className="row-more visible-file-action" title="Delete" onClick={() => { setSelected([file.path]); runBulk("delete"); }}><Trash2 size={15} />Delete</button></span></div>)}</div>}<div className="file-tip"><ShieldCheck size={15} /><span>Each server has its own Docker volume. Uploads keep their original filenames; only the configured startup command runs.</span></div></div>;
}
...
function UsersView({ onAction }: { onAction: (action: string) => void }) {
  return <div className="subview"><div className="subview-heading"><div><span className="eyebrow">Access control</span><h2>Users & permissions</h2><p>Choose who can access this server and what they can do.</p></div><button className="primary-button" onClick={() => onAction("Invite user flow opened")}><UserPlus size={15} />Invite user</button></div><div className="data-table user-table"><div className="data-row data-head"><span>User</span><span>Role</span><span>2FA</span><span>Last active</span><span /></div>{[{ n: "Alex Morgan", e: "alex@acme.dev", initials: "AM", role: "Owner", two: true, active: "Just now" }, { n: "Jamie Chen", e: "jamie@acme.dev", initials: "JC", role: "Developer", two: true, active: "12 min ago" }, { n: "Deploy bot", e: "automation@aerion.run", initials: "DB", role: "Service account", two: false, active: "18 min ago" }].map((user) => <div className="data-row" key={user.e}><span className="member"><span className="member-avatar">{user.initials}</span><span><strong>{user.n}</strong><small>{user.e}</small></span></span><span className="role-chip">{user.role}</span><span>{user.two ? <Check className="table-check" size={15} /> : <span className="muted-text">Off</span>}</span><span className="muted-text">{user.active}</span><button className="row-more" onClick={() => onAction(`${user.n} permissions opened`)}><MoreHorizontal size={16} /></button></div>)}</div></div>;
}

function BackupsView({ onAction }: { onAction: (action: string) => void }) {
  return <div className="subview"><div className="subview-heading"><div><span className="eyebrow">Recovery points</span><h2>Backups</h2><p>Snapshots are encrypted and stored in a separate region.</p></div><button className="primary-button" onClick={() => onAction("Backup created")}><Plus size={15} />Create backup</button></div><div className="backup-banner"><div className="backup-banner-icon"><ShieldCheck size={19} /></div><div><strong>Automatic backups are healthy</strong><span>Last successful backup was 12 minutes ago. Retention: 14 days.</span></div><button onClick={() => onAction("Backup retention settings opened")}>Configure <ArrowUpToLine size={13} /></button></div><div className="data-table backup-table"><div className="data-row data-head"><span>Backup</span><span>Size</span><span>Created</span><span>Status</span><span /></div>{["signal-bot-2026-09-11-0942", "signal-bot-2026-09-11-0800", "signal-bot-2026-09-10-2000"].map((backup, i) => <div className="data-row" key={backup}><span className="backup-name"><FileArchive size={16} />{backup}</span><span className="muted-text">{i === 0 ? "184.2 MB" : "181.9 MB"}</span><span className="muted-text">{i === 0 ? "12 min ago" : `${i + 2} hours ago`}</span><StatusPill>Ready</StatusPill><div className="row-actions"><button className="row-more" onClick={() => onAction("Backup download started")}><Download size={15} /></button><button className="row-more" onClick={() => onAction("Backup restore flow opened")}><RotateCcw size={15} /></button></div></div>)}</div></div>;
}

function StartupView({ onAction, serverName }: { onAction: (action: string) => void; serverName: string }) {
  const utils = trpc.useUtils(); const startup = trpc.node.startup.useQuery({ name: serverName }, { enabled: serverName !== "No server selected" }); const [command, setCommand] = useState(""); const [envText, setEnvText] = useState("");
  useEffect(() => { if (startup.data) { setCommand(startup.data.command); setEnvText(Object.entries(startup.data.env || {}).map(([k,v]) => k + "=" + v).join("\n")); } }, [startup.data]);
  const save = trpc.node.updateStartup.useMutation({ onSuccess: () => { void startup.refetch(); onAction("Startup settings saved and server restarted"); }, onError: e => onAction(e.message) });
  const saveStartup = () => { const env = Object.fromEntries(envText.split("\n").map(line => line.split("=")).filter(parts => /^[A-Z_][A-Z0-9_]*$/.test(parts[0] || "")).map(parts => [parts[0], parts.slice(1).join("=")])); save.mutate({ name: serverName, runtime: startup.data?.runtime || "nodejs", command, env }); };
  return <div className="subview"><div className="subview-heading"><div><span className="eyebrow">Runtime configuration</span><h2>Startup & variables</h2><p>Change the command that runs inside the Docker container and restart safely.</p></div><button className="primary-button" onClick={saveStartup} disabled={save.isPending}><Check size={15} />{save.isPending ? "Saving..." : "Save & restart"}</button></div><div className="startup-card"><div className="startup-card-head"><div className="startup-command-icon"><Terminal size={17} /></div><div><strong>Startup command</strong><span>Egg runtime: {startup.data?.runtime ?? "Loading..."}</span></div></div><textarea className="startup-command-editor" value={command} onChange={e => setCommand(e.target.value)} placeholder="npm install --omit=dev && npm start" spellCheck={false} /><div className="startup-help">Examples: <code>npm start</code> · <code>python3 main.py</code> · <code>nginx -g 'daemon off;'</code></div></div><div className="startup-card"><div className="startup-card-head"><div className="startup-command-icon"><KeyRound size={17} /></div><div><strong>Environment variables</strong><span>One KEY=value per line. Secrets are passed only to the container.</span></div></div><textarea className="startup-command-editor variable-editor" value={envText} onChange={e => setEnvText(e.target.value)} placeholder="DISCORD_TOKEN=your-secret\nPORT=3000" spellCheck={false} /><div className="startup-help">Use uppercase names and never commit secrets to your uploaded files.</div></div><div className="warning-note"><AlertTriangle size={16} /><div><strong>Saving restarts this server.</strong><span>Install dependencies in the command and keep long-running bots in the foreground.</span></div></div></div>;
}
function NetworkView({ onAction }: { onAction: (action: string) => void }) {
  return <div className="subview"><div className="subview-heading"><div><span className="eyebrow">Public endpoints</span><h2>Network & allocations</h2><p>Manage ports, domains, and private connectivity for this server.</p></div><button className="primary-button" onClick={() => onAction("Allocation added")}><Plus size={15} />Create allocation</button></div><div className="network-grid"><div className="allocation-card allocation-primary"><div className="allocation-head"><div className="allocation-icon"><Network size={17} /></div><StatusPill>Primary</StatusPill></div><strong>iad1.aerion.run:30124</strong><span>Public · TCP · IPv4</span><div className="allocation-foot"><button onClick={() => onAction("Allocation address copied")}><Copy size={13} />Copy address</button><button onClick={() => onAction("Domain attach flow opened")}><Link2 size={13} />Attach domain</button></div></div><div className="allocation-card"><div className="allocation-head"><div className="allocation-icon allocation-violet"><Cloud size={17} /></div><StatusPill tone="violet">Private</StatusPill></div><strong>signal-bot.internal:8080</strong><span>Private network · HTTP</span><div className="allocation-foot"><button onClick={() => onAction("Private URL copied")}><Copy size={13} />Copy URL</button><button onClick={() => onAction("Private network opened")}><Network size={13} />Inspect</button></div></div></div><div className="domain-card"><GlobeIcon /><div><strong>Custom domains</strong><span>Point your own domain at a public allocation.</span></div><button onClick={() => onAction("Custom domain flow opened")}>Add domain <Plus size={13} /></button></div></div>;
}
...
export default function Home() {
  ...
  const renderPanel = () => {
    ...
    if (activePanel === "Startup") return <StartupView {...props} serverName={activeServer.name} />;
