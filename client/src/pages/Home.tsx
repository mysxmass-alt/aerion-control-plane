...
function FilesView({ onAction, serverName }: { onAction: (action: string) => void; serverName: string }) {
  const fileInput = useRef<HTMLInputElement>(null); const utils = trpc.useUtils();
  const filesQuery = trpc.files.list.useQuery({ serverName }, { refetchInterval: 4000 });
  const uploadMutation = trpc.files.upload.useMutation({ onSuccess: () => { void utils.files.list.invalidate({ serverName }); onAction("File uploaded"); }, onError: e => onAction(e.message) });
  const extractMutation = trpc.files.extract.useMutation({ onSuccess: () => { void utils.files.list.invalidate({ serverName }); onAction("Archive extracted"); }, onError: e => onAction(e.message) });
  const actionMutation = trpc.files.action.useMutation({ onSuccess: () => { void utils.files.list.invalidate({ serverName }); onAction("File operation completed"); }, onError: e => onAction(e.message) });
  const uploadFile = (file: File) => { if (file.size > 25 * 1024 * 1024) return onAction("Files must be 25 MB or smaller"); const reader = new FileReader(); reader.onload = () => { if (typeof reader.result === "string") uploadMutation.mutate({ serverName, fileName: file.name, relativePath: file.webkitRelativePath || file.name, mimeType: file.type || "application/octet-stream", size: file.size, dataUrl: reader.result }); }; reader.readAsDataURL(file); };
  const files = filesQuery.data?.files ?? [];
  const fileAction = (action: "move" | "rename" | "copy" | "delete", source: string) => { if (action === "delete" && !window.confirm("Delete " + source + " permanently?")) return; const destination = action === "delete" ? "" : window.prompt(action === "rename" ? "New file path" : "Destination path", source); if (action !== "delete" && !destination) return; actionMutation.mutate({ serverName, action, source, destination: destination || undefined }); };
  return <div className="subview"><div className="subview-toolbar"><div className="pathbar"><Folder size={15} /><span>/</span><strong>home/container</strong></div><div className="toolbar-actions"><input ref={fileInput} type="file" multiple hidden onChange={e => { for (const file of Array.from(e.target.files ?? [])) uploadFile(file); e.currentTarget.value=""; }} /><button className="outline-button" onClick={() => fileInput.current?.click()} disabled={uploadMutation.isPending}><Upload size={14} />{uploadMutation.isPending ? "Uploading..." : "Upload files or ZIP"}</button><button className="outline-button" onClick={() => onAction("Create folders by moving or uploading files") }><Plus size={14} />New folder</button></div></div><div className="file-tools"><div className="file-search"><Search size={14} /><input placeholder="Filter files..." /></div><span>{filesQuery.isLoading ? "Loading container files..." : files.length + " files · live Docker volume"}</span></div>{filesQuery.isLoading ? <div className="storage-loading"><RefreshCw size={17} className="spin-icon" />Loading files...</div> : files.length === 0 ? <div className="empty-state storage-empty"><div><Folder size={21} /></div><strong>Container is empty</strong><p>Upload a ZIP or project files, then extract the archive and edit your startup command.</p><button onClick={() => fileInput.current?.click()}>Upload project <Upload size={14} /></button></div> : <div className="data-table file-table"><div className="data-row data-head"><span>Name</span><span>Size</span><span>Modified</span><span>Actions</span></div>{files.map(file => <div className="data-row" key={file.path}><span className="file-name"><span className="file-icon file-icon-code"><File size={16} /></span><strong>{file.path}</strong></span><span className="muted-text">{formatBytes(file.size)}</span><span className="muted-text">{new Date(file.modifiedAt).toLocaleString()}</span><span className="file-row-actions">{file.path.toLowerCase().endsWith(".zip") && <button className="row-more" title="Extract ZIP" onClick={() => extractMutation.mutate({ serverName, fileName: file.path })}><FileArchive size={15} /></button>}<button className="row-more" title="Rename" onClick={() => fileAction("rename", file.path)}>Rename</button><button className="row-more" title="Move" onClick={() => fileAction("move", file.path)}>Move</button><button className="row-more" title="Copy" onClick={() => fileAction("copy", file.path)}>Copy</button><button className="row-more" title="Delete" onClick={() => fileAction("delete", file.path)}><Trash2 size={15} /></button></span></div>)}</div>}<div className="file-tip"><ShieldCheck size={15} /><span>Files are stored on the live Docker volume. ZIP extraction is handled on the node.</span></div></div>;
}
