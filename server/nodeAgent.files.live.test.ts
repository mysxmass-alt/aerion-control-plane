...
describe("live node file workflows", () => {
  afterAll(async () => {
    if (process.env.AERION_NODE_AGENT_URL && process.env.AERION_NODE_AGENT_TOKEN) await deleteNodeServer(testServer).catch(() => undefined);
  });

  it("creates a container, uploads a ZIP, and extracts it into the volume", async () => {
    if (!process.env.AERION_NODE_AGENT_URL || !process.env.AERION_NODE_AGENT_TOKEN) return;
    ...
    const temp = await mkdtemp(path.join(tmpdir(), "aerion-test-"));
    try {
      await writeFile(path.join(temp, "hello.txt"), "hello from Aerion\n");
